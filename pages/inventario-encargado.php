...
<button type="B1"  class="btn btn-primary  " style="border-radius: 0px;" onclick="myFunctionB1()">Movimientos del día</button>
  <button type="B2" hidden  class="btn btn-primary " style="border-radius: 0px;" onclick="myFunctionB2()">Movimientos de la semana</button>
  <button type="B3" hidden class="btn btn-primary " style="border-radius: 0px;" onclick="myFunctionB3()">Movimientos del mes</button>

</div>

<?php                  

/* query para mostrar cantidad de cajas existentes 
                      ...
                      echo '<h4 class="m-2 font-weight-bold text-primary">'.($row['CANTIDAD']).' '.($row['PRODUCTO']).'</h4>';
...
/* query Para mostrar Inventario Permanente
                      $query = "SELECT * FROM almacen WHERE  CITY LIKE '{$_SESSION['CITY']}%'";
                      $result = mysqli_query($db, $query) or die (mysqli_error($db));
        
                      while ($row = mysqli_fetch_assoc($result)) {
                     
                      
                      echo '<h4 class="card-header m-2 font-weight-bold text-primary"> '.($row['NAME']).' '.($row['QTY_STOCK']).'  '.($row['DATE_STOCK_IN']).'</h4>';
...
<?php                  
                      ...
                      echo '<tr>';
                      echo '<td>'.($row['NAME']).'</td>';
                      echo '<td>'.($row['QTY_STOCK']).(' '). ($row['EXT']).'</td>';
/*
                      echo  '<td>';
                      if ($row['TIPO'] == 1) {
                        echo "AGREGÓ";
                 ...
                 echo '</td>';*/
                      ...
                      //echo '<td>'. date("Y-m-d H:i:s", strtotime($row['FECHA_ENTREGA'])).'</td>';
                      echo '<td>'. date("d/m/Y", strtotime($row['FECHA_ENTREGA'])).'</td>';
                      echo '<td>'.($row['MONTO']).(' '). ($row['EXT']).'</td>';
