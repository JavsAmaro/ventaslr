...
$opt = "<input placeholder='Buscar Cliente' list='options' style='border-radius: 0px;width: 50%;'id='customer' onchange='javascript:AddValue()' onchange='javascript:myFunction(event)' name='customer'  >
<datalist id='options' >
...
<?php
                  ...
                  $Cliente_Sucursal =  $row['CUST_ID'];
					  ...
					  echo 'Cliete Sucursal:'.$Cliente_Sucursal;
  ?>
 ...
 $(document).on('click keyup','.mis-checkboxes,.mis-adicionales',function() {
   calcular();
      ...
      $(function () {
		...
		// alert("2. theForm" + form);
        $("#metodoPago").change(function () {
            var selectedText = $(this).find("option:selected").text();
            var selectedValue = $(this).val();
        });
		
		alert("2. selectedValue" + selectedValue);
		
    });

var btnEnviar = document.getElementById('btnEnviar');
alert("2. btnEnviar" + btnEnviar);
...
fx.addEventListener('input', function () {
	...
	alert(fx.value + " " + resultado.innerText + " " + error);
	alert(resultado.innerText);
...
{
	var clienteSucursal = <?php echo $Cliente_Sucursal; ?>;
	var value = 0;
	if(clienteSucursal != ""){
